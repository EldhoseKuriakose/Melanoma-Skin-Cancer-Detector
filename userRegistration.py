...
def insert():
	...
	if age.isdigit() and '@' in mailid and '.' in mailid and contact.isdigit(): 
	    	if(password1==password2):
	        	...
	        	tkMessageBox.showinfo("Status", "Successfully Inserted")
	        	clr()
	    	else:
	        	tkMessageBox.showinfo("Status", "Password not matched")
        else:
	        	tkMessageBox.showinfo("Status", "Plese enter valid Phone number/ Age / Email")
	
    else:
        	tkMessageBox.showinfo("Status", "Enter Complete Details")
...
##		btn_cancel.grid(row=11,column=1)
		...
		top.mainloop()
	#except:
		#tkMessageBox.showinfo("Info","An error occured")
